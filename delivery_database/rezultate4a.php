...
$sql ="SELECT DISTINCT c.numec, p.numep , p.idp, c.oras 
					FROM Livrari l INNER JOIN Proiecte p ON l.idp = p.idp
					INNER JOIN Componente c ON l.idc = c.idc
					WHERE c.oras = p.oras and c.oras='$oras'";//.$nume;

if(is_object($conn->query($sql))){
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
       echo " Proiectul ". $row["idp"]."( ". $row["numep"].")". ", si componenta ". $row["numec"]." din orasul ".$row["oras"]."<br>";
   } 
} else {
   echo "0 results";
}}
