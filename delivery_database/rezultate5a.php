...
$sql ="select numec, masa, oras from componente where (oras='$oras' and masa<=all(select masa from componente where oras='$oras'))";
//.$nume;
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
       echo "Componenta ". $row["numec"]. ", cu masa ".$row["masa"] ." din orasul ".$row["oras"]."<br>";
   } 
} else {
   echo "0 results";
}
